...
class EndpointManagementTool:
    ...
    description = (
        "Manage API endpoints, create routes, test HTTP requests, and validate schemas"
    )
    category = "api"
    ...
    async def _create_endpoint(self, args: dict) -> dict:
        ...
        if not name or not path:
            return {"success": False, "error": "Missing required fields: name, path"}

        # Check for duplicate
        if name in self.endpoints:
            return {"success": False, "error": f"Endpoint already exists: {name}"}
    ...
    async def _delete_endpoint(self, args: dict) -> dict:
        ...
        if name in self.endpoints:
            ...
            return {"success": False, "error": f"Endpoint not found: {name}"}
    ...
    async def _test_endpoint(self, args: dict) -> dict:
        ...
        if name in self.endpoints:
            ...
            mock_responses = {
                "status": "ok",
                "data": {"endpoint": name, "method": method, "path": endpoint["path"]},
            }
