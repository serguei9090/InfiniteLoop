...
class BatchOperationsTool:
    ...
    async def _process_single_file(self, path: str, operation: str) -> dict:
        ...
        if operation == "read":
            ...
            try:
                ...
                return {"success": True, "output": len(lines), "type": "line_count"}
            except Exception as e:
                return {"success": False, "error": str(e)}
            ...
            try:
                ...
                return {"success": True, "output": words, "type": "word_count"}
            except Exception as e:
                return {"success": False, "error": str(e)}
    ...
    async def _bulk_rename(self, args: dict) -> dict:
        ...
        for file_path in files:
            try:
                new_path = (
                    Path(new_name) / Path(file_path).name if new_name else file_path
                )
