...
class AtomicTask(BaseModel):
    ...
    description: str = Field(description="A detailed description of the task to perform")
    target_file: str = Field(description="The primary file to modify or read")
    verification_step: str = Field(description="The command or step to run to verify the task succeeded")

class MissionPlan(BaseModel):
    mission_objective: str = Field(description="The overall goal of the mission")
    tasks: List[AtomicTask] = Field(description="The sequence of tasks to complete the mission")
