...
class ToolRegistry:
            ...
            try:
                ...
                if spec and spec.loader:
                    ...
                    if run_func:
                        ...
                        tool_def.function = lambda **kwargs: asyncio.run(main_func(**kwargs))
                ...
                logger.info(f"Hot-loaded tool: {name} (total hot-loads: {self.hot_load_count})")
        ...
        for name, tool_def in self._tools.items():
            if category and tool_def.module_path:
                ...
                tools_list.append({
                    "name": name,
                    "description": tool_def.description,
                    "category": (module_path.parent.name or "backend") if (module_path := Path(tool_def.module_path)) else "backend",
                    "is_dynamic": tool_def.is_dynamic,
                    "created_at": tool_def.created_at,
                    "last_used": tool_def.last_used
                })
    ...
    def get_adaptation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "categories": {cat: len([t for t in self._tools.values() if (Path(t.module_path) or Path()).parent.name == cat]) 
    ...
    def get_tool_descriptions(self) -> str:
        ...
        return "\n".join(descriptions) if descriptions else "No custom tools registered yet."
