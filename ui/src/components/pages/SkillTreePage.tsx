import { GitBranch, Book, Cpu, HelpCircle, Lock } from "lucide-react";

// Mock data representing nodes in each quadrant
const skillsNodes = [
  { id: "s1", label: "File System", x: 150, y: 150, status: "active" },
  { id: "s2", label: "Read/Write", x: 250, y: 100, status: "active", parent: "s1" },
  { id: "s3", label: "Delete/Move", x: 250, y: 200, status: "active", parent: "s1" },
  { id: "s4", label: "Advanced I/O", x: 380, y: 150, status: "learning", parent: "s2" },
];

const knowledgeNodes = [
  { id: "k1", label: "TypeScript", x: 150, y: 150, status: "active" },
  { id: "k2", label: "React", x: 280, y: 100, status: "active", parent: "k1" },
  { id: "k3", label: "Tailwind", x: 280, y: 200, status: "active", parent: "k1" },
  { id: "k4", label: "NextJS", x: 400, y: 100, status: "locked", parent: "k2" },
];

const modulesNodes = [
  { id: "m1", label: "Context Engine", x: 150, y: 150, status: "active" },
  { id: "m2", label: "Tree-sitter", x: 250, y: 100, status: "active", parent: "m1" },
  { id: "m3", label: "Auto-Evolver", x: 250, y: 200, status: "learning", parent: "m1" },
];
  ...
  return (
    <div className="h-full w-full bg-surface-50 p-8 pb-24 flex flex-col relative overflow-hidden font-sans">
      ...
      <header className="mb-8 shrink-0 relative z-10 text-center">
        <h1 className="text-4xl font-black text-slate-800 tracking-tight uppercase">Evolution Matrix</h1>
        <p className="text-slate-500 mt-2 font-mono text-sm tracking-widest">AGENT CAPABILITY TOPOLOGY</p>
      </header>
      ...
      <div className="flex-1 relative bg-white rounded-3xl border border-slate-200 shadow-lg overflow-hidden">
        ...
        <div className="absolute top-0 left-0 w-1/2 h-1/2 p-8 relative">
          <QuadrantLabel title="SKILLS" icon={<GitBranch />} color="text-emerald-500" bgColor="bg-emerald-50" align="left" />
          <NodeCanvas nodes={skillsNodes} colorClass="emerald" direction="bottom-right" />
        </div>

        {/* Quadrant 2: Top Right - KNOWLEDGE */}
        <div className="absolute top-0 right-0 w-1/2 h-1/2 p-8 relative">
          <QuadrantLabel title="KNOWLEDGE" icon={<Book />} color="text-blue-500" bgColor="bg-blue-50" align="right" />
          <NodeCanvas nodes={knowledgeNodes} colorClass="blue" direction="bottom-left" />
        </div>

        {/* Quadrant 3: Bottom Left - MODULES */}
        <div className="absolute bottom-0 left-0 w-1/2 h-1/2 p-8 relative">
          <QuadrantLabel title="MODULES" icon={<Cpu />} color="text-purple-500" bgColor="bg-purple-50" align="left" bottom />
          <NodeCanvas nodes={modulesNodes} colorClass="purple" direction="top-right" />
        </div>

        {/* Quadrant 4: Bottom Right - UNKNOWN */}
        <div className="absolute bottom-0 right-0 w-1/2 h-1/2 p-8 relative">
          <QuadrantLabel title="AWAITING DATA" icon={<HelpCircle />} color="text-slate-400" bgColor="bg-slate-50" align="right" bottom />
          <NodeCanvas nodes={emptyNodes} colorClass="slate" direction="top-left" />
        </div>
      </div>
    </div>
  );
...
function QuadrantLabel({ title, icon, color, bgColor, align, bottom }: any) {
  return (
    <div className={`absolute ${bottom ? 'bottom-8' : 'top-8'} ${align === 'left' ? 'left-8' : 'right-8'} flex items-center gap-3 opacity-60`}>
      {align === 'right' && <h2 className={`text-2xl font-black tracking-widest ${color}`}>{title}</h2>}
      <div className={`p-3 rounded-xl border border-slate-100 ${bgColor} ${color}`}>
        {icon}
      </div>
      {align === 'left' && <h2 className={`text-2xl font-black tracking-widest ${color}`}>{title}</h2>}
    </div>
  );
}

function NodeCanvas({ nodes, colorClass, direction }: { nodes: any[], colorClass: string, direction: string }) {
  ...
  return (
    <div className="absolute inset-0">
        ...
        {nodes.map(node => {
          ...
          return (
             <line
                ...
                strokeWidth="3"
                strokeDasharray={node.status === 'learning' || node.status === 'locked' ? '6,6' : 'none'}
      ...
      {nodes.map(node => (
        ...
        >
          <div className={`w-12 h-12 rounded-xl border-2 flex items-center justify-center transition-all bg-white z-10 shadow-sm
          ...
          `}>
            {node.status === 'locked' ? <Lock size={20} /> : <div className="w-4 h-4 rounded-full bg-current" />}
          </div>
          <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-md border whitespace-nowrap shadow-sm
