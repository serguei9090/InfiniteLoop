...
const mockSkills = [
  ...
  { id: "fs", label: "File System Ops", status: "active", icon: GitBranch, parent: "core" },
  { id: "read", label: "Read/List", status: "active", icon: GitCommit, parent: "fs" },
  { id: "write", label: "Edit/Create", status: "active", icon: GitCommit, parent: "fs" },
  { id: "exec", label: "Bash Execution", status: "active", icon: GitBranch, parent: "core" },
  { id: "web", label: "Web Browsing", status: "learning", icon: GitMerge, parent: "core", progress: 65 },
  { id: "vision", label: "Computer Vision", status: "locked", icon: Lock, parent: "core" },
];
...
function SkillNode({ skill }: { skill: any }) {
  ...
  const getStatusStyle = () => {
    switch (skill.status) {
      case "active": return "bg-white border-emerald-500 text-emerald-600 shadow-[0_0_15px_rgba(16,185,129,0.2)]";
      case "learning": return "bg-white border-amber-500 text-amber-600 shadow-[0_0_15px_rgba(245,158,11,0.2)]";
      case "locked": return "bg-slate-100 border-slate-300 text-slate-400 border-dashed";
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className={`w-12 h-12 rounded-xl border-2 flex items-center justify-center relative transition-all hover:scale-105 cursor-pointer ${getStatusStyle()}`}>
        <Icon size={20} />
